...
router.get('/:slug', (req, res) => {
  try {
    ...
    const schedules = scheduleRows.map(r => ({
      dow: r.dow,
      slots: safeParseJSON(r.slots, [])
    }));

    // Parse page_config de forma segura
    const pageConfig = safeParseJSON(business.page_config, {});
    ...
    console.error('Error en GET /:slug:', err);
...
router.get('/:slug/slots', (req, res) => {
  try {
    ...
    scheduleRows.forEach(r => {
      scheduleMap[r.dow] = safeParseJSON(r.slots, []);
    });
    ...
    console.error('Error en GET /:slug/slots:', err);
